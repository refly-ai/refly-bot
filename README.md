...
- **🍎 Mac / 🐧 Linux**:
  ```bash
  curl -sSL https://ghproxy.net/https://raw.githubusercontent.com/refly-ai/refly-bot/main/run.sh?version=1 | APP_ID=xxx APP_SECRET=xxx REFLY_API_KEY=xxx bash
  ```
- **🪟 Windows (PowerShell)**:
  ```powershell
  $env:APP_ID="xxx"; $env:APP_SECRET="xxx"; $env:REFLY_API_KEY="xxx"; iwr https://ghproxy.net/https://raw.githubusercontent.com/refly-ai/refly-bot/main/run.bat?version=1 -OutFile run.bat; .\run.bat
  ```
- **🪟 Windows (CMD)**:
  ```batch
  set APP_ID=xxx& set APP_SECRET=xxx& set REFLY_API_KEY=xxx& curl -L https://ghproxy.net/https://raw.githubusercontent.com/refly-ai/refly-bot/main/run.bat?version=1 -o run.bat && run.bat
  ```
